	...
	for (block = 0; block < blockCount; block++) {
		...
		for (xOffset = i = 0; i < DCT_BLOCK; i++, xOffset += width) {
			...
			for (j = yStart, k = 0; j < yEnd; j++, k++) {
				...
				dctY[i][k] = inPixel.y - 128;
				dctCb[i][k] = inPixel.cb - 128;
				dctCr[i][k] = inPixel.cr - 128;
			}
		}

		// DCT on block
		ippiDCT8x8Fwd_32f_C1I(dctY);
		ippiDCT8x8Fwd_32f_C1I(dctCb);
		ippiDCT8x8Fwd_32f_C1I(dctCr);

		// Quantization
		for (i = 0; i < DCT_BLOCK; i++) {
			for (j = 0; j < DCT_BLOCK; j++) {
				dctY[i][j] = round(dctY[i][j] / kK1Table[i][j]);
				dctCb[i][j] = round(dctCb[i][j] / kK2Table[i][j]);
				dctCr[i][j] = round(dctCr[i][j] / kK2Table[i][j]);
			}
		}
		...
		for (xOffset = i = 0; i < DCT_BLOCK; i++, xOffset += width) {
			...
			for (j = yStart, k = 0; j < yEnd; j++, k++) {
				outPixel.y = dctY[i][k];
				outPixel.cb = dctCb[i][k];
				outPixel.cr = dctCr[i][k];
...
ImageYCC *idctTransformation(ImageYCC *image)
{
	...
	for (block = 0; block < blockCount; block++) {
		...
		for (xOffset = i = 0; i < DCT_BLOCK; i++, xOffset += width) {
			...
			yEnd = yStart + DCT_BLOCK;
			for (j = yStart, k = 0; j < yEnd; j++, k++) {
				dctY[i][k] = image->data[j].y;
				dctCb[i][k] = image->data[j].cb;
				dctCr[i][k] = image->data[j].cr;
			}
		}
		...
		// IDCT on block
		ippiDCT8x8Inv_32f_C1I(dctY);
		ippiDCT8x8Inv_32f_C1I(dctCb);
		ippiDCT8x8Inv_32f_C1I(dctCr);
		for (i = 0; i < DCT_BLOCK; i++) {
			for (j = 0; j < DCT_BLOCK; j++) {
				dctY[i][j] += 128;
				dctCb[i][j] += 128;
